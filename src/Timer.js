...
import GAME_SETTINGS from "../game-settings.json";
...
export default class Timer extends Phaser.GameObjects.Container {
    ...
    getGameFinalTime() {
        const activeGameScene = window.App?.gameScene || this.scene?.game?.scene?.getScene('Game');
        if (activeGameScene?.SETTINGS?.game && typeof activeGameScene.SETTINGS.game.gameFinalTime === 'number') {
            return activeGameScene.SETTINGS.game.gameFinalTime;
        }
        if (GAME_SETTINGS?.game && typeof GAME_SETTINGS.game.gameFinalTime === 'number') {
            return GAME_SETTINGS.game.gameFinalTime;
        }
        return 0;
    }
    ...
    _reset() {
        const finalTime = this.getGameFinalTime();
        this._elapsedMs            = 0;
        this._triggered            = false;
        this._running              = false;
        this._lastDisplayedSeconds = Math.ceil((finalTime * 10) / 1000);
    ...
    _tick(time, delta) {
        ...
        const remainingMs = Math.max(0, finalTime * 10 - this._elapsedMs);
        ...
        if (!this._triggered && this._elapsedMs >= finalTime * 10) {
    ...
    _animateToFinalPosition(onComplete) {
        ...
        const destScale = SETTINGS.globalScale * (isPortrait ? orientationScale.portrait : orientationScale.landscape);
        const destPx = SETTINGS.container.portraitX;
        const destPy = SETTINGS.container.portraitY;
        const destLx = SETTINGS.container.landscapeX;
        const destLy = SETTINGS.container.landscapeY;

        const currentX = this.x;
        const currentY = this.y;

        // Set target alignment immediately so coordinate system is unified
        this.setAlign(align);

        const alignX = Utils.getAlignX(this);
        const alignY = Utils.getAlignY(this);

        const startPx = currentX - alignX;
        const startPy = currentY - alignY;
        const startLx = currentX - alignX;
        const startLy = currentY - alignY;
        ...
        this.lx = startLx;
        this.ly = startLy;
        this.pScaleX = startScale;
        this.pScaleY = startScale;
        this.lScaleX = startScale;
        this.lScaleY = startScale;
        ...
        this.scene.tweens.add({
            ...
            onUpdate: () => {
                ...
                const curPx = startPx + (destPx - startPx) * p;
                const curPy = startPy + (destPy - startPy) * p;
                const curLx = startLx + (destLx - startLx) * p;
                const curLy = startLy + (destLy - startLy) * p;
                const curScale = startScale + (destScale - startScale) * p;

                this.px = curPx;
                this.py = curPy;
                this.lx = curLx;
                this.ly = curLy;
                this.pScaleX = curScale;
                this.pScaleY = curScale;
                this.lScaleX = curScale;
                this.lScaleY = curScale;

                if (this.scene?.game?.size?.isPortrait) {
                    this.setCustomPosition(curPx, curPy);
                } else {
                    this.setCustomPosition(curLx, curLy);
                }
            ...
            onComplete: () => {
                ...
                this.lx = destLx;
                this.ly = destLy;
                this.pScaleX = destScale;
                this.pScaleY = destScale;
                this.lScaleX = destScale;
                this.lScaleY = destScale;
                const activePortrait = this.scene?.game?.size?.isPortrait;
                this.setCustomPosition(activePortrait ? destPx : destLx, activePortrait ? destPy : destLy);
                this.setScale(destScale);
